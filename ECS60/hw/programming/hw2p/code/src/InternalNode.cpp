...
void InternalNode::removeFromThis(BTreeNode *ptr, int pos)
{  // pos is where the ptr should go, guaranteed count < internalSize at start
//    did not consider only one item case

  children[pos] = ptr;
  keys[pos] = ptr->getMinimum();

  for(int i = pos + 1; i <= count - 2; i++)
  {
    children[i] = children[i + 1];
    keys[i] = keys[i + 1];
  } // shift to the right to make room at pos
...
BTreeNode* InternalNode::removePtr(int pos) {
...
void InternalNode::borrowFromLeft() {
  int last = leftSibling->getCount();
  BTreeNode* ptr = ((InternalNode*) leftSibling)->removePtr(last);
  insert(ptr);
}

void InternalNode::borrowFromRight() {
  BTreeNode* ptr = ((InternalNode*) rightSibling)->removePtr(0);
  insert(ptr);
}

InternalNode* InternalNode::mergeLeft(InternalNode* sibling) {
  InternalNode* ptr = new InternalNode(internalSize, leafSize, parent, sibling->leftSibling, rightSibling);

  for (int j = 0; j < sibling->getCount(); j++) {
    ptr->children[ptr->count] = sibling->children[j];
    ptr->keys[ptr->count] = sibling->keys[j];
    ptr->children[ptr->count++]->setParent(ptr);
  }

  for (int i = 0; i < count; i++) {
    ptr->children[ptr->count] = children[i];
    ptr->keys[ptr->count] = keys[i];
    ptr->children[ptr->count++]->setParent(ptr);
  }
...
InternalNode* InternalNode::mergeRight(InternalNode* sibling) {
  InternalNode* ptr = new InternalNode(internalSize, leafSize, parent, leftSibling, sibling->rightSibling);
...
{  // to be written by students
  ...
  if (ptr->getMinimum() < keys[pos])
    pos--;
  removeFromThis(ptr, pos);

  int threshold = (internalSize % 2) ? internalSize / 2 + 1 : internalSize / 2;
  ...
  if (count < threshold) {
    if (leftSibling) {
      if (leftSibling->getCount() > threshold) {
        ...
        return mergeLeft((InternalNode*) leftSibling);
      }
    }
      ...
      if (rightSibling->getCount() > threshold) {
        ...
        return mergeRight((InternalNode*) rightSibling);
      }
    }
