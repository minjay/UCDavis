...
```{r global_options, include=FALSE, echo=FALSE, cache=TRUE}
options(width=70)
knitr::opts_chunk$set(warning=FALSE, message=FALSE, tidy=TRUE, fig.path='Figs/', cache=TRUE)
```
...
# extract data
name_with_alias <- data_na_tm_ci[, c('na.id', 'na.name', 'an.alias')]
names(name_with_alias) <- c('id', 'name', 'alias')
...
## 13 and 14 are using big dataset
...
actor_compare <- sapply(movie_actor_count[movie_actor_count$actor_count == 20, ][1:100, ][['actor_id']], calculate_first_actor_count)
